...
stat_df = pd.DataFrame(data_list, columns=['operation_time', 'max_appearance', 'mean', 'variance', 'selected_value'])
...
for i in range(19, 20):
    ...
    z = np.polyfit(x, y, i)
    p = np.poly1d(z)
    ax1.plot(x, p(x), label=str(i))
    for row in stepped_event_stream.groupby('operation_time'):
        if 1500 < row[0] < 4800:
            mean = row[1]['page_no'].mean()
            point_loss = abs(mean - p(row[0]))
...
# print(Loss_list)
# print("The minimum loss power is:%d" % Loss_list.index(min(Loss_list)))
ax1.legend()
